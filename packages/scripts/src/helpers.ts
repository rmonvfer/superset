 ...
 */
const promptForVariableAction = async (
	key: string,
 ...
 */
export const buildEnvFileContent = (
...
export const writeEnvFile = async (
	...
	try {
		...
		if (fileExists && existingVars.size > 0) {
			...
			const resolvedVars = await resolveVariableConflicts(
				existingVars,
				newVars,
			);
			...
			const writeSpinner = ora(
				`Writing updated ${label} .env to ${filePath}`,
			).start();
			try {
			...
			const writeSpinner = ora(
				`Writing new ${label} .env to ${filePath}`,
			).start();
