...
import {
	createCodeForCoupon,
	createLegacyCoupon,
} from "@superset/stripe/src/scripts/production/coupon";
...
export const seedLegacySubscriptions = async () => {
	...
	for (const email of emails) {
		if (!email) continue;
		const { id: stripePromotionCodeId, code: stripePromotionCode } =
			await createCodeForCoupon(stripe, stripeCouponId, email);
