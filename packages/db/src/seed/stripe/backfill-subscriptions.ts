...
import {
	subscriptions,
	type Subscription,
} from "@/schema/subscription/subscription";
import { usageRecords } from "@/schema/subscription/usage";
import { users } from "@/schema/user/user";
import { db } from "@superset/db/src/client";
import { UsageType } from "@superset/models";
import { SubscriptionStatus } from "@superset/stripe";
import { createStripeClient } from "@superset/stripe/src/client";
...
const insertRateLimit = async (
	...
	const remainingUsage = Math.max(
		0,
		item.subscription.price.monthlyMessageLimit - usageCount,
	);
...
export const backfillSubscriptions = async () => {
	...
	for (const item of stripeItems) {
		...
		await db.transaction(async (tx) => {
			...
			console.log(
				`Inserting rate limit for subscription ${item.subscription.id}`,
			);
			// Insert rate limit based on usage records and subscription price
			await insertRateLimit(tx, item);
			console.log(
				`Rate limit inserted for subscription ${item.subscription.id}`,
			);
		});
	}
};
