import { Color } from "@superset/utility";
...
export const generateGradientCSS = (gradient: GradientState): string => {
	...
	const stopStrings = sortedStops.map(
		(stop) =>
			`${applyOpacity(stop.color, stop.opacity ?? 100)} ${stop.position}%`,
	);
...
export const parseGradientFromCSS = (
	cssValue: string,
): GradientState | null => {
	try {
		const normalized = cssValue.trim();

		const extractGradientParams = (
			css: string,
			gradientType: string,
		): string | null => {
...
export const Gradient: React.FC<GradientProps> = ({
	...
	useEffect(() => {
		const handleMouseMove = (event: MouseEvent) => {
			if (!dragState.isDragging || !dragState.stopId || !trackRef.current)
				return;
	...
	const addStop = useCallback(() => {
		try {
			...
			if (selectedStopId) {
				const selectedStop = gradient.stops.find(
